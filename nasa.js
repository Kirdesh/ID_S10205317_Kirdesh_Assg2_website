const apiKey = "nUK2KShgbswgR3VGmcTcs9uQYplAgNo87XKrYmi3";
const apodUrl = "https://api.nasa.gov/planetary/apod";
const trackingUrl = "https://api.nasa.gov/neo/rest/v1/feed";


function handleTrackingData({element_count, near_earth_objects}, trackingElement, trackingTable){
    

    trackingElement.innerHTML = Object.keys(near_earth_objects).map(date=>{
    ...
    if(trackingElement.innerHTML === ""){
        trackingTable.className = "striped hide"
    } else{
        trackingTable.className = "striped";
    }
...
function contentLoaded(){
    ...
    .then(data=>{
        ...
        if(data.media_type === "image"){
            ...
            media = `<div class="video-container">
                        <iframe src="${data.hdurl}" width="560" height="315"></iframe>
                    </div>`
        }
    ...
    start.addEventListener("change", function(){
        ...
        fetch(`${trackingUrl}?start_date=${this.value}&api_key=${apiKey}`)
        .then(res=>res.json())
        .then(data=>handleTrackingData(data, trackingElement, trackingTable))
        .catch(handleError);
